    ...
    createStructure: (options) => {
        ...
        switch(type){
            ...
            case lineTypes.FAM_RECORD:
                ...
                // console.log(line[2].value, line[2].value.replaceAll("@", ""))
                struct = new Family(line[0], line[2].value.replaceAll("@", ""), line[4], "", []);
                break;

            // Individual Record
            case lineTypes.INDI_RECORD:
                // Remove @-sign from beginning and end of XREF
                struct = new Individual(line[0], line[2].value.replaceAll("@", ""), line[4], "", []);
                break;
            ...
            case lineTypes.NO_LINEVAL:
                // Remove @-sign from beginning and end of XREF
                struct = new Structure(line[0], line[2].value.replaceAll("@", ""), line[4], "", []);
                break;
            
            // Structure of the form: LEVEL D XREF D TAG D LINEVAL EOL
            default:
                // Remove @-sign from beginning and end of XREF
                struct = new Structure(line[0], line[2].value.replaceAll("@", ""), line[4], line[6], []);
