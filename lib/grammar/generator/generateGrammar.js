...
class GrammarGenerator{
    ...
    async generateParser(){
        ...
        try{
            if(await fsExists(this.path + "nearley")){
                recordNeFiles = await fs.readdir(this.path + "nearley");
        ...
        for(const file of recordNeFiles){
            ...
            if(fileName.length === 2 && fileName[1] === "ne"){
                ...
                for(const syn of syntaxNeFile){
                    fileStr += `@include "../nearley/syntax/${syn}"\n`;
                }
                ...
                // read grammar of current file
                const grammar = await fs.readFile(`${this.path}nearley/${file}`, { encoding: 'utf8' });

                // append grammar of current file to dummy-file
                fs.appendFile(this.dummyPath, grammar);

                // compile dummy .ne-file
                await exec(`npx nearleyc ${this.dummyPath} -o ${this.path}parser/${fileName[0]}Parser.js`);
            }
