...
module.exports = {
  ...
  createStructure: (params) => {
    ...
    if (!params.line && !params.type) throw new Error('data and type required!');

    const type = params.type;
    const line = params.line.map(part => {
      if (part) {
    ...
    switch (type) {
      ...
      case lineTypes.NO_XREF:
        ...
        if (params.lineValType === dataTypes.ListEnum) {
          const enumValues = gedcomEnumTypes[params.enumType];
          ...
          for (const val of line[3].split(',')) {
            ...
            if (val && !enumValues.includes(val.trim())) {
              ...
              throw new GedcomSyntaxError(`LineValue ${val} is not listed in ${params.enumType}!\n${line[0]} ${line[2]} ${line[3]}\n` + spaces + '^');
            }
            offset += val.length + 1;
          }
          // check lineValue if it is from type Enum
        } else if (params.lineValType === dataTypes.Enum) {
          const enumValues = gedcomEnumTypes[params.enumType];
          // line[3] is equivalent to thelineValue
    ...
    return {
      ...
      line: lineObject,
      type: type,
      lineValType: params.lineValType ? params.lineValType : null,
      superstructFound: false,
      substructs: [],
      checkCardinalityOf: params.checkCardinalityOf
    };
  },
  // add substructures to given superstructure
