...
async function main () {
  ...
  const dataset = await gedcomParser.parseGedFile('test/sampleData/DatasetTest/DateExamples/dateExample2.ged');
  ...
  const dateObj = fam.getSubstructuresByTag("MARR")[0].getSubstructuresByTag("DATE")[0].getDateObject();
  console.log(dateObj);
  ...
  //newTest();
  // const dataset2 = await gedcomParser.parseGedFile("test/sampleData/ExampleFamilySearchGEDCOMFiles/maximal70_without_extensions.ged");
  await ausarbeitungAblauf();
}
...
async function ausarbeitungAblauf(){
  const gedcomParser = new GedcomParser();
