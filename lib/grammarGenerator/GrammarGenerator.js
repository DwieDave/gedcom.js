...
const { lineTypes, dataTypes, gedcomEnumTypes, continuationTypes } = require('../util/Constants');
const Records = require('./structures/Records.js');
...
class GrammarGenerator {
  ...
  async generateStructureInfo (path) {
    const stringifyReplacer = ['info', 'lineType', 'level', 'lineValType', 'tag', 'substructs'];
    // STRUCTURE INFO
    let infoStr = 'module.exports = {\n';

    infoStr += '\n// HEADER';
    this.headerRecord.rules.forEach(rule => {
    ...
    infoStr += '\n// STRUCTURE TYPES';
    this.structureTypes.rules.forEach(rule => {
      infoStr += `\n"${rule.uri}": ${JSON.stringify(rule, null, '\t')},`;
    });
    ...
    infoStr += '\n// SUBSTRUCTURES';
    this.substructures.rules.forEach(rule => {
      infoStr += `\n"${rule.uri}": ${JSON.stringify(rule, null, '\t')},`;
    });
