...
class GedcomCardinalityError extends Error {
    constructor(errorTag, line, minimumError) {
        if(minimumError){
            super(`Missing gedcom line\nNo "${errorTag}" tag in Structure "${line.level} ${line.xref ? "@" + line.xref + "@ " : ""}${line.tag}${line.lineVal ? " " + line.lineVal : ""}"`);
        }else{
...
module.exports = {ParsingError, GedcomSyntaxError, GedcomCardinalityError, GedcomResultUndefinedError, GedcomLevelError};