...
const {lineTypes, dataTypes, emptyHeader, emptyTrailer} = require('./util/Constants.js');
const {GedcomCardinalityError, GedcomSyntaxError, DatasetError, Warning} = require('./util/Errors.js');
...
module.exports = class Dataset {
    ...
    addSubstructWithXrefDatatype(struct){
        if(this.substructsWithXrefDatatype[struct.lineVal]){
            let val = this.substructsWithXrefDatatype[struct.lineVal]
            val.push(struct);
    ...
    removeSubstructWithXrefDatatype(struct){
        let entries = this.substructsWithXrefDatatype[struct.lineVal];
        if(entries.length === 1){
