    ...
    createRecord(parsedObject, superstruct){
        ...
        switch(type){
            ...
            case lineTypes.HEADER:
                struct = new Structure.Header(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                break;

            // Family Record
            case lineTypes.FAM_RECORD:
                struct = new Structure.Family(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                break;

            // Individual Record
            case lineTypes.INDI_RECORD:
                struct = new Structure.Individual(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                break;
            
            // Multimedia Record
            case lineTypes.OBJE_RECORD:
                struct = new Structure.Multimedia(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                break;

            // Repository Record
            case lineTypes.REPO_RECORD:
                struct = new Structure.Repository(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                break;

            // SharedNote Record
            case lineTypes.SNOTE_RECORD:
                struct = new Structure.SharedNote(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                break;
            
            // Source Record
            case lineTypes.SOUR_RECORD:
                struct = new Structure.Source(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                break;
            
            // Submitter Record
            case lineTypes.SUBM_RECORD:
                struct = new Structure.Submitter(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                break;
            ...
            default:
                ...
                if(parsedObject.lineValType === dataTypes.ListText || parsedObject.lineValType === dataTypes.ListEnum){
                    ...
                    struct = new Structure.Structure(line.level, line.xref, line.tag, lineVal, line.EOL, []);
                }else{
                    struct = new Structure.Structure(line.level, line.xref, line.tag, line.lineVal, line.EOL, []);
                }
        }
