...
module.exports = class Structure {
  ...
  constructor (uri, level, xref, tag, lineVal, lineValType, EOL, substructures, datasetRef) {
    ...
    this.lineValType = lineValType || dataTypes.NoLineVal;
    ...
    //  -> It is recommended that tools provide an interface to allow users to filter data on export such that certain RESN structure payload
    //  entries result in the RESN structure and its superstructure being removed from the export
    // TO-DO: Implement support for restrictionValues
  ...
  // TO-DO: flag "resolveSubstructures" -> if not set, return SUBSTRUCT, not uri
  getPossibleSubstructs (checkCardinalityFlag = false) {
    const possibleSubstructs = {};
    ...
    for (const [uri, cardinality] of Object.entries(entries)) {
      ...
      if (uri.startsWith('g7:')) {
        ...
        const substructures = [{ uri: uri, cardinality: cardinality }];
  ...
  getSubstructuresBy (searchVal, searchType, recursive, dataType = false) {
    ...
    for (const sub of this.substructures) {
      ...
      if (recursive) {
        const foundRecursive = sub.getSubstructuresBy(searchVal, searchType, recursive, dataType);
