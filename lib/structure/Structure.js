...
module.exports = class Structure {
    ...
    addSubstructure(options) {
        const structInfo = StructureInfo[options.uri];
        const structure = {};

        // set structure information on basis of given options and StructureInformation 
        structure["type"] = structInfo["lineType"];
        structure["lineValType"] = structInfo["lineValType"];
        structure["line"] = {
            level: this.level + 1,
            xref: options.xref ? options.xref : "",
            tag: structInfo["tag"],
            lineVal: options.lineVal ? options.lineVal : ""
        };
        structure.substructs = options.substructs ? options.substructs : [];
        ...
        try{
            ...
            // check if xref of new substruct is defined in dataset
            if(this.lineValType === dataTypes.Xref){
                this.datasetReference.checkXref(structure.xref, structure);
            }
        }catch(e){
    ...
    setXref(xref){
        ...
        if(this.associatedRecord === null){
            ...
            try{
                ...
                this.datasetReference.addXref(this);
