...
export class Family extends Structure  {
    constructor(line) {
        super(line);
        this.substructuresWithCardinalityOne = new Set([GedcomTags.HUSBAND, GedcomTags.WIFE]);
    }
    ...
    // Problem: Kardinalität
    // -> reguläre Ausdrücke erstellen und zusätzlich überprüfen
    // -> Dataset (alphabetisch) sortieren sodass eindeutige Reihenfolge festgelegt ist
    //      -> ist Reihenfolge innerhalb eines Records von Bedeutung? 
    checkSyntax(){
        // set of substructures with cardinality {0:1} that are already found
        let alreadyFound = new Set();
        
        // iterate over all direct substructures 
        for(const sub of this.substructures){
            // Create a Parser object from family grammar.
            const parser = new nearley.Parser(nearley.Grammar.fromCompiled(grammar));

            // check if current substructure has cardinality {0:1} and is already present in this substructure
            if(this.substructuresWithCardinalityOne.has(sub.tag)){
                if(alreadyFound.has(sub.tag)) throw new GedcomSyntaxError(`Cardinality Error: multiple ${sub.tag} in one FAM record are not allowed!`);
                else alreadyFound.add(sub.tag);
            }
            // check grammar of current substructure
            try{
                console.log(`Try to parse:\n${sub.toString()}`);
                parser.feed(sub.toString());
